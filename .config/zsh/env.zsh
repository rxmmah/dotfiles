# Custom key binding to open Vim with a temporary file
export QT_QPA_PLATFORMTHEME=qt6ct  # or qt5ct
...
export XDG_BOOKS="$HOME/Books"

# Function to set XDG_SESSION_TYPE based on the current session
set_xdg_session_type() {
  if [ "$XDG_SESSION_TYPE" = "wayland" ]; then
    export XDG_SESSION_TYPE="wayland"
  elif [ "$XDG_SESSION_TYPE" = "x11" ]; then
    export XDG_SESSION_TYPE="x11"
  else
    # Fallback to detecting session type
...
# Call the function to set the XDG_SESSION_TYPE
set_xdg_session_type
